(function(){
  ...
  const overview=document.getElementById('visao-geral')||corpo.querySelector('.obra-texto');if(overview&&!overview.id)overview.id='visao-geral';
  ...
  const header=document.querySelector('.work-hero,.obra-header');if(!header)return;header.insertAdjacentElement('afterend',nav);
  const links=[...nav.querySelectorAll('a')],sections=items.map(([id])=>document.getElementById(id));
