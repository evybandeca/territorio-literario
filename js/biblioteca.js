(function(){
  ...
  function render(){
    ...
    lista.innerHTML=dados.map((o,i)=>{
      const profundo=corpus[o.id],mentions=(profundo?.place_mentions||[]).length,entities=(profundo?.place_entities||[]).length,tom=tons[Math.abs((o.movimento||'').split('').reduce((a,c)=>a+c.charCodeAt(0),0))%tons.length],reader=readerRegistry[o.id],hasReader=Boolean(reader),obraHref=`obra.html?id=${encodeURIComponent(o.id)}`,readerHref=`leitura.html?obra=${encodeURIComponent(o.id)}`;
