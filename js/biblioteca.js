(function(){
  ...
  const params=new URLSearchParams(location.search),autorInicial=params.get('autor'),qInicial=params.get('q'),movInicial=params.get('movimento'),ordInicial=params.get('ordem');
  if(contador){contador.setAttribute('aria-live','polite');contador.setAttribute('aria-atomic','true')}
  try{
    const saved=JSON.parse(localStorage.getItem(STORE)||'{}');
    if(qInicial!==null)busca.value=qInicial;
    else if(!autorInicial)busca.value=saved.busca||'';
    if(movInicial!==null&&movimentos.includes(movInicial))mov.value=movInicial;
    else if(!autorInicial&&movimentos.includes(saved.movimento))mov.value=saved.movimento||'';
    if(ordInicial!==null&&['ano','titulo','autor'].includes(ordInicial))ord.value=ordInicial;
    else if(!autorInicial&&['ano','titulo','autor'].includes(saved.ordem))ord.value=saved.ordem;
  }catch(_){ }
  let urlFrame=0;
  const syncUrl=()=>{
    ...
    urlFrame=requestAnimationFrame(()=>{
      ...
      if(busca.value.trim())next.searchParams.set('q',busca.value.trim());else next.searchParams.delete('q');
      if(mov.value)next.searchParams.set('movimento',mov.value);else next.searchParams.delete('movimento');
      if(ord.value&&ord.value!=='ano')next.searchParams.set('ordem',ord.value);else next.searchParams.delete('ordem');
      history.replaceState(null,'',`${next.pathname}${next.search}${next.hash}`);
    });
  };
  const persist=()=>{if(!autorInicial){try{localStorage.setItem(STORE,JSON.stringify({busca:busca.value,movimento:mov.value,ordem:ord.value}))}catch(_){ }}syncUrl()};
  ...
  function announceResults(count){document.dispatchEvent(new CustomEvent('tl:results-updated',{detail:{surface:'biblioteca',count}}))}
  function render(){
    ...
    syncRail();
    if(!dados.length){lista.innerHTML='<div class="empty-state"><strong>Nenhuma obra encontrada.</strong><p>Tente remover os filtros ou buscar por outro título ou autor.</p><button type="button" data-clear-library>Limpar filtros</button></div>';lista.setAttribute('aria-busy','false');announceResults(0);return}
    ...
    lista.setAttribute('aria-busy','false');announceResults(dados.length);
  }
