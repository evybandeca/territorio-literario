(function(global){
  ...
  function loadScript(src){
    if(document.querySelector(`script[data-corpus-src="${src}"]`))return Promise.resolve();
    return new Promise((resolve,reject)=>{
      const script=document.createElement('script');
      script.src=src;script.async=false;script.dataset.corpusSrc=src;
      script.onload=()=>resolve();
      script.onerror=()=>reject(new Error(`Falha ao carregar corpus: ${src}`));
      document.head.appendChild(script);
    });
  }
  async function loadCorpusById(id){
